...
new $.rgbmultiselector(this,myPrefs);});}});$.rgbmultiselector=function(input,prefs){var KEY={UP:38,DOWN:40,DEL:46,TAB:9,RETURN:13,ESC:27,COMMA:188,PAGEUP:33,PAGEDOWN:34,BACKSPACE:8};var $sSelect=$(input);var selectId=$sSelect.attr("id");var inputId=selectId+"_rgbmultiselect";var optionsId=inputId+"_container";var iframeId=inputId+"_iframe";var baseCheckboxId=optionsId+"_checkbox";var selectWidth=parseInt($sSelect.outerWidth(),10);var leaveFieldTimeout=null;var blockSubmit=false;var hasFocus=0;var keypressTimeout=null;var optionsNumMatching=0,optionsOneSelectedMatch=null,optionsOneSelectedMatchType="";var optionsKeyboardCurrentId=null,optionsKeyboardCurrentType=null;var clearInputField=true;var unselectedOriginalHeight=null;var $sInput=buildInputField();var inputWidth=parseInt($sInput.width(),10);var $sOptions=buildOptionsContainer();$(document.body).append($sOptions);var $sIframe=buildIE6Iframe();var selectCache=getSelectOptions();updateFieldText();if(prefs.buildOptionsInBackground){$(document).ready(function(){setTimeout(function(){buildOptions();if($.browser.msie){$sOptions.find(".jquery_rgbmultiselect_options_selected .jquery_rgbmultiselect_options_item_checkbox").attr("checked","checked");}},10);});}
...
buildOptions();showOptions();$(this).removeClass("jquery_rgbmultiselect_blurred");if(clearInputField){clearInputField=false;$(this).val("");}});$sInput.click(function(){hasFocus++;if(hasFocus>1&&!$sOptions.is(":visible")){showOptions();}});$sInput.blur(function(){hasFocus=0;leaveFieldTimeout=setTimeout(function(){leaveField();},200);});$sInput.bind(($.browser.opera?"keypress":"keydown")+".rgbmultiselect",function(e){hasFocus=1;switch(e.keyCode){case KEY.ESC:hideOptions();break;case KEY.RETURN:e.preventDefault();blockSubmit=true;selectOnEnterAndReset();e.stopPropagation();return false;case KEY.UP:if(!$sOptions.is(":visible")){showOptions();}else{keyGo(-1);}
...
function filterOptions(){var str=$.trim($sInput.val());var strParts=str.split(/ +/);if(optionsNumMatching==1){optionsOneSelectedMatch.parent().removeClass("jquery_rgbmultiselect_options_item_singlefiltered");var value=getItemId(optionsOneSelectedMatch,optionsOneSelectedMatchType);selectCache[value].filtered=false;}
...
return originalText;}
function selectOnEnterAndReset(){if(optionsNumMatching==1){if(numOptionsSelected()>=prefs.maxSelections&&prefs.maxSelections>-1){return;}
...
$sInput.focus().val("");filterOptions();updateUnselectedHeight();}else{if(optionsKeyboardCurrentId!==null&&(optionsKeyboardCurrentType=="clearlist"||!selectCache[optionsKeyboardCurrentId].filtered)){if(numOptionsSelected()>=prefs.maxSelections&&prefs.maxSelections>-1&&optionsKeyboardCurrentType!="clearlist"&&!selectCache[optionsKeyboardCurrentId].selected){return;}
...
function showOptions(){positionOptionsContainer();if(!$sOptions.is(":visible")){$sOptions.show();}
...
function buildOptionsContainer(){var optContainer=$e("div");optContainer.hide().addClass("jquery_rgbmultiselect_options_container").attr("id",optionsId).css({width:inputWidth+"px"});return optContainer;}
...
fieldText=fieldText.replace(/%o/,replacementsO);fieldText=fieldText.replace(/%c/,replacementsC);fieldText=fieldText.replace(/%a/,replacementsA);$sInput.val(fieldText);}}
...
return c;}
function getSelectOptions(){var data={};$sSelect.find("OPTION").each(function(){var value=$(this).val();var rel=$(this).attr("rel");var text=$(this).text();var selected=$(this).is(":selected");data["_"+value]={rel:rel,text:text,selected:selected,filtered:false,disabled:false};});return data;}
function buildInputField(){var existingInputField=$("#"+inputId);var inputField;if(existingInputField.size()==1){inputField=existingInputField;}else{inputField=$e("input");inputField.attr({type:"text",id:inputId,name:inputId}).css({width:selectWidth+"px"});}
inputField.addClass("jquery_rgbmultiselect_input").attr("autocomplete","off").val(prefs.inputDefaultText);$sSelect.after(inputField).hide();return inputField;}
...
function objTypeIs(o,type){return selectCache[o].rel&&selectCache[o].rel.indexOf(type)>-1;}
function objTypeNot(o,type){return!selectCache[o].rel||selectCache[o].rel.indexOf(type)==-1;}
